...
void ABoard::BeginPlay()
{
    ...
    BlockExplosivo = GetWorld()->SpawnActor<ABlockExplosivo>(FVector(1000, 1000, 1000), FRotator(0, 0, 0));
    Ingeniero = GetWorld()->SpawnActor<AIngenieroBlock>(AIngenieroBlock::StaticClass());
...
void ABoard::Builder()
{
    ...
    if (contador == 1)
    {
        Ingeniero->SetConstructorBlock(BlockExplosivo);
        Ingeniero->ConstruirBloque();

        GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Yellow, FString::Printf(TEXT("%i El contador"), contador));
        ABlockGeneral* BlockGeneral = Ingeniero->GetBlockGeneral();
        BlockGeneral->BloqueCaracteristicas();

    }
    else if (contador == 2) {
        Ingeniero->SetConstructorBlock(BlockMadera);
        Ingeniero->ConstruirBloque();
